...
function initKeyboardNavigation(container) {
  ...
  // Detect number of columns dynamically
  const firstTile = tiles[0];
  const tileWidth = firstTile.offsetWidth;
  const containerWidth = container.offsetWidth;
  const cols = Math.max(1, Math.round(containerWidth / tileWidth));
  ...
  document.addEventListener('keydown', (e) => {
    // Only respond if a tile or the body is focused (so we can start right away)
    const active = document.activeElement;
    if (active !== document.body && !tiles.includes(active)) return;
    ...
    switch (e.key) {
      ...
      case 'ArrowUp':
        e.preventDefault();
        currentIndex = (currentIndex - cols + tiles.length) % tiles.length;
        break;
      case 'ArrowDown':
        e.preventDefault();
        currentIndex = (currentIndex + cols) % tiles.length;
        break;
      case 'Enter':
      case ' ':
        e.preventDefault();
